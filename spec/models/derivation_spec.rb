...
RSpec.describe Derivation, type: :model do
  ...
  it 'requires a unique source_publication for each derived_publication' do
    derivation_1 = FactoryGirl.create(:derivation)
    derivation_2 = FactoryGirl.build(
                     :derivation,
                     source_publication: derivation_1.source_publication,
                     derived_publication: derivation_1.derived_publication
                   )

    expect(derivation_2).not_to be_valid
    expect(derivation_2.errors[:source_publication]).to(
